...
export default function NationalitySelect({
  ...
  const selected = useMemo(
    () => COUNTRIES.find((c) => c.nationality === value) || null,
    [value],
  );
  ...
  const pick = (c) => {
    onChange?.({ target: { name, value: c.nationality, type: "text" } });
    setOpen(false);
  };
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      >
        ...
        >
          {selected ? selected.nationality : placeholder}
        </span>
          ...
          >
            ...
            <div className="max-h-60 overflow-y-auto py-1">
              {filtered.length === 0 ? (
                ...
                filtered.map((c) => {
                  const isSel = c.nationality === value;
                  return (
                    <button
                      key={c.code}
                      type="button"
