...
export default function AdminCaseworkers() {
  ...
  const validateReset = () => {
    ...
    else if (resetForm.new_password.length < 8)
  ...
  const handleToggle = async (row) => {
    setToggleId(row.id);
    try {
      const res = await toggleCaseworkerStatus(row.id);
      showToast({
  ...
  const handleResetSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await resetCaseworkerPassword(modal.data.id, {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Manage caseworker accounts and workload access
          </p>
        </div>
              ...
              {!loading && caseworkers.length === 0 ? (
                ...
                caseworkers.map((user, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-600 max-w-[140px] truncate">
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <span
                        className={`px-2.5 py-1 rounded-full text-[11px] font-black ${
                          ROLE_CHIPS[
                            (user.Role?.name || "caseworker").toLowerCase()
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-800">
                {modal.data ? departmentLabel(modal.data) : ""}
              </p>
            </div>
            <div>
              <p className="text-xs font-black text-gray-400 uppercase tracking-wider mb-1">
              ...
              <span
                className={`inline-block px-2.5 py-1 rounded-full text-[11px] font-black ${
                  ROLE_CHIPS[
                    (modal.data?.Role?.name || "caseworker").toLowerCase()
                  ] ?? "bg-gray-100 text-gray-500"
                }`}
              >
            ...
            {modal.data?.performance && (
              <div className="col-span-2 pt-2 border-t border-gray-100">
                ...
                <p className="text-sm text-gray-700">
                  Total {modal.data.performance.totalCases ?? 0} · Completed{" "}
                  {modal.data.performance.completedCases ?? 0} · Completion{" "}
                  {modal.data.performance.completionRate ?? 0}%
                </p>
              </div>
            )}
          </div>
