      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
            ...
            <button className="ml-auto inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-black text-white hover:bg-primary-dark transition shadow-sm">
              <Download size={13} />
              Download
            </button>
          </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(workerStats.byVisaType).map(([type, count]) => (
                      <span
                        key={type}
                        className="px-2.5 py-1 rounded-lg bg-gray-50 border border-gray-200 text-xs font-black text-secondary"
                      >
          ...
          {selectedReport === "financial" && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard
                label="Total Revenue"
                value={`£${financial.totalRevenue.toLocaleString("en-GB")}`}
              />
              <StatCard
                label="Paid This Period"
                value={`£${financial.paidThisMonth.toLocaleString("en-GB")}`}
                color="green"
              />
              <StatCard
                label="Pending"
                value={`£${financial.pendingPayments.toLocaleString("en-GB")}`}
                color="amber"
              />
              <StatCard
                label="Avg Processing"
                value={`${financial.avgProcessingTime} days`}
              />
            </div>
          )}
