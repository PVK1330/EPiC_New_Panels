...
import PageHero, {
  HeroButton,
  HeroGhostButton,
} from "../../components/superadmin/PageHero";
import {
...
const SuperadminTeam = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setModules(
        loadedModules.length ? loadedModules : PLATFORM_MODULES_FALLBACK,
      );
    } catch (e) {
      const msg =
        e?.response?.data?.message || e.message || "Failed to load team";
      setLoadError(msg);
  ...
  const confirmRevokeAccess = async () => {
    ...
    try {
      setSaving(true);
      await teamService.updateTeamMember(revokeTarget.id, {
        status: "inactive",
      });
      ...
      toast.error(
        e?.response?.data?.message || e.message || "Failed to revoke access",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleRestoreAccess = async (member) => {
    try {
      ...
      toast.error(
        e?.response?.data?.message || e.message || "Failed to restore access",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveMember = async () => {
    ...
    try {
      ...
      await teamService.updateTeamMember(editingMember.id, {
        ...
        last_name: editMemberForm.last_name.trim(),
        role_id: Number(editMemberForm.role_id),
      });
  ...
  const confirmDeleteMember = async () => {
    ...
    try {
      setSaving(true);
      await teamService.deleteTeamMember(deleteMemberTarget.id);
      ...
      toast.error(
        e?.response?.data?.message || e.message || "Failed to delete member",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleEditRole = (role) => {
    setEditingRole(role);
    const perms = {};
    (role.perms || role.moduleIds || []).forEach((p) => {
      perms[p] = true;
    });
    setSelectedPermissions(perms);
    setRoleFormName(
      role.name?.replace(/^platform_/, "").replace(/_/g, " ") || role.name,
    );
  ...
  const handleSaveRole = async () => {
    const moduleIds = Object.keys(selectedPermissions).filter(
  ...
  const handleInviteSubmit = async () => {
    ...
    try {
      ...
      if (welcome?.sent) {
        ...
        toast.success(`Member created. Temporary password: ${tempPw}`, {
          duration: 14000,
        });
      } else {
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      {activeTab === "Members" ? (
        <>
          ...
          >
            ...
            >
              <div
                className={`w-11 h-11 rounded-lg bg-blue-500 text-white flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}
              >
              ...
              <div>
                ...
                <h4 className="text-2xl font-black text-secondary tracking-tight">
                  {stats.total
                    ? Math.round((stats.mfa_enabled / stats.total) * 100)
                    : 0}
                  %
                </h4>
              </div>
            ...
            >
              <div
                className={`w-11 h-11 rounded-lg bg-green-500 text-white flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}
              >
            ...
            >
              <div
                className={`w-11 h-11 rounded-lg bg-purple-500 text-white flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}
              >
            ...
            >
              <div
                className={`w-11 h-11 rounded-lg bg-amber-500 text-white flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}
              >
                <RiTimeLine size={20} />
              </div>
              <div>
                ...
                <h4 className="text-2xl font-black text-secondary tracking-tight uppercase">
                  Live
                </h4>
              </div>
            </motion.div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
            <div className="px-4 py-2.5 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/30">
              <div className="flex items-center gap-3">
                ...
                <span className="px-2 py-0.5 bg-secondary text-white text-[9px] font-black uppercase tracking-widest rounded shadow-sm">
                  {filteredTeam.length} Active Members
                </span>
              </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-4 py-10 text-center text-xs font-bold text-gray-400 uppercase tracking-widest"
                      >
                        Loading team…
                      </td>
                    </tr>
                      ...
                      {filteredTeam.map((member, idx) => (
                        ...
                        >
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-9 h-9 rounded-lg text-white flex items-center justify-center font-semibold text-base shadow-sm group-hover:scale-110 transition-all ${
                              ...
                              <div>
                                <p className="font-bold text-secondary text-xs flex items-center gap-2">
                                  {member.name}
                                  {member.mfa && (
                                    <span className="px-1.5 py-0.5 bg-green-50 text-green-700 border border-green-100 rounded text-xs font-semibold">
                                      2FA On
                                    </span>
                                  )}
                                </p>
                          ...
                          <td className="px-4 py-2.5">
                            <span
                              className={`px-2.5 py-0.5 rounded-full text-sm font-semibold border ${
                                member.status === "Active"
                              ...
                              <div className="w-16 h-1 bg-gray-100 rounded-full mt-1.5 overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width:
                                      member.status === "Active"
                                        ? `${(member.modules / MODULE_COUNT) * 100}%`
                                        : "0%",
                                  }}
                                />
                              </div>
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              {member.status === "Active" ? (
                                <button
                                  ...
                                  title="Revoke access"
                                  className="text-sm font-semibold text-red-600 hover:text-red-700 hover:underline transition-all flex items-center gap-1.5 hover:bg-red-50/50 px-2 py-1 rounded"
                                >
                                  <RiLockLine size={14} />
                                  Revoke Access
                                </button>
                                ...
                                <button
                                  ...
                                  className="text-sm font-semibold text-green-600 hover:text-green-700 hover:underline transition-all flex items-center gap-1.5 hover:bg-green-50/50 px-2 py-1 rounded"
                                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {roles.map((role) => (
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-secondary">
                      {role.display_name ||
                        formatPlatformRoleName(role.name, role.id)}
                    </h4>
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full block mt-1 border ${role.type === "System" ? "bg-primary/5 text-primary border-primary/10" : "bg-secondary/5 text-secondary border-secondary/10"}`}
                    >
                      {role.type} Role
                    </span>
                  </div>
              ...
              <div className="relative z-10 flex items-center gap-2 mt-auto pt-4 border-t border-gray-50">
                ...
                >
                  <RiEditLine className="inline mr-1" size={14} />
                  Edit
                </Button>
                {role.type !== "System" && (
                  <Button
          ...
          >
            <RiAddLine
              size={32}
              className="mb-2 text-gray-300 group-hover:text-primary transition-colors"
            />
      ...
      <Modal
        ...
        subtitle={
          editingRole
            ? `Updating permissions for ${editingRole.name}.`
            : "Define a custom administrative role with pre-set module access."
        }
        maxWidth="max-w-2xl"
        footer={
          <div className="flex justify-end gap-2 w-full">
            ...
            <Button
              ...
              className="px-8 py-2 text-sm font-bold shadow-sm"
            >
              {saving
                ? "Saving…"
                : editingRole
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {modules.map((mod) => (
                ...
                >
                  <div>
                    <p
                      className={`text-sm font-semibold ${selectedPermissions[mod.id] ? "text-primary" : "text-secondary"}`}
                    >
                      {mod.label}
                    </p>
                    <p
                      className={`text-xs font-bold mt-0.5 ${selectedPermissions[mod.id] ? "text-primary/70" : "text-gray-400"}`}
                    >
                      {mod.description}
                    </p>
                  </div>
        ...
        footer={
          <div className="flex justify-end gap-2 w-full">
            ...
            <Button
              ...
              className="px-6 py-2 text-sm font-bold bg-red-600 border-red-600 hover:bg-red-700 shadow-sm"
            >
              Delete Role
            </Button>
          </div>
      ...
      >
        <div className="flex flex-col items-center text-center py-4">
          ...
          <p className="text-sm font-bold text-secondary mb-2">
            Delete {editingRole?.name}?
          </p>
          <p className="text-sm text-gray-400 font-bold px-4 leading-relaxed">
            All members currently assigned to this role will lose their
            administrative permissions immediately.
          </p>
        </div>
      </Modal>
          ...
          {revokeTarget && (
            <>
              ...
              <p className="text-sm text-gray-600 font-medium px-4 leading-relaxed">
                ...
                ? This user will no longer be able to access assigned modules.
              </p>
            </>
          )}
        </div>
      ...
      <Modal
        ...
        subtitle="Update the member's name and assigned role."
        maxWidth="max-w-lg"
        footer={
          <div className="flex justify-end gap-2 w-full">
            <Button
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Input
                ...
                onChange={(e) =>
                  setEditMemberForm((p) => ({ ...p, first_name: e.target.value }))
                }
              />
              <Input
                label="Last name"
                ...
                onChange={(e) =>
                  setEditMemberForm((p) => ({ ...p, last_name: e.target.value }))
                }
              />
            </div>
            {editingMember && (
              <Input label="Work Email" value={editingMember.email} disabled />
            )}
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setEditMemberForm((p) => ({ ...p, role_id: e.target.value }))
                }
                className="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-lg text-sm font-bold text-secondary outline-none focus:ring-1 focus:ring-primary/20 appearance-none"
              >
      ...
      >
        <div className="flex flex-col items-center text-center py-6">
          ...
          {deleteMemberTarget && (
            <>
              <p className="text-sm font-bold text-secondary mb-4">
                Delete {deleteMemberTarget.name}?
              </p>
              <p className="text-sm text-gray-600 font-medium px-4 leading-relaxed">
                ...
                ({deleteMemberTarget.email}) from the platform team.
              </p>
            </>
          )}
        </div>
        ...
        <div className="space-y-6 py-1">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setInviteForm((p) => ({ ...p, role_id: e.target.value }))
                }
                className="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-lg text-sm font-bold text-secondary outline-none focus:ring-1 focus:ring-primary/20 appearance-none"
              >
                <option value="">Select role</option>
          ...
          <div className="p-4 bg-primary/5 rounded-xl border border-primary/10 flex gap-3">
            ...
            <p className="text-sm text-primary font-bold leading-relaxed">
              The recipient will be granted access only to the modules defined
              in the selected role. Custom roles can be updated anytime in the
              Roles tab.
            </p>
          </div>
        </div>
