...
import { RiNotification3Line, RiCheckDoubleLine, RiFilter3Line, RiSettings4Line, RiHistoryLine, RiCloseLine } from 'react-icons/ri';
...
const SuperadminNotifications = () => {
  ...
  const [readIds, setReadIds] = useState(new Set([4, 5, 6, 7]));

  const initialNotifications = [
    { id: 1, title: 'New Organisation Signed Up', desc: 'Acme Corp just completed registration. They are pending manual review and approval.', time: '2 mins ago', type: 'success' },
    { id: 2, title: 'Server Load Alert', desc: 'Database cluster CPU usage exceeded 85% for more than 5 minutes. Consider scaling resources.', time: '1 hour ago', type: 'warning' },
    { id: 3, title: 'Payment Failed', desc: 'Subscription renewal failed for tenant Global Migrate Pro. Automatic retry scheduled in 24 hours.', time: '3 hours ago', type: 'error' },
    { id: 4, title: 'Platform Update', desc: 'System maintenance scheduled for v2.4 deployment on Saturday at 02:00 AM UTC.', time: '1 day ago', type: 'info' },
    { id: 5, title: 'API Key Created', desc: 'A new production API key was generated by the Administrator account.', time: '2 days ago', type: 'info' },
    { id: 6, title: 'User Quota Exceeded', desc: 'London Legal Partners attempted to add users beyond their Starter plan quota (5/5).', time: '3 days ago', type: 'warning' },
    { id: 7, title: 'Backup Completed', desc: 'Daily automated database snapshot completed successfully. 12.4GB stored.', time: '4 days ago', type: 'success' },
  ];
  ...
  const filtered = activeTab === 'All' 
    ...
    : initialNotifications.filter(n => !readIds.has(n.id));
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div className="divide-y divide-gray-50">
            {filtered.map((notif) => (
              <div 
                ...
                className={`p-4 flex items-start gap-4 transition-all cursor-pointer group relative ${!readIds.has(notif.id) ? 'bg-primary/[0.02]' : 'hover:bg-gray-50/50'}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-0.5">
                    <h3 className={`text-sm tracking-tight ${!readIds.has(notif.id) ? 'font-black text-secondary' : 'font-bold text-gray-600'}`}>
                      {notif.title}
                    </h3>
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest shrink-0">{notif.time}</span>
                  </div>
                  <p className={`text-xs ${!readIds.has(notif.id) ? 'text-gray-600 font-semibold' : 'text-gray-400 font-medium'} leading-relaxed max-w-4xl truncate`}>
                    {notif.desc}
                  </p>
                </div>
              </div>
          ...
          {filtered.length === 0 && (
            <div className="p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-[10px] text-gray-400 mt-1 font-bold uppercase tracking-widest">No new system alerts to display</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="py-4 space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{selectedNotification.time}</p>
              </div>
            </div>
