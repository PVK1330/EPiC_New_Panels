...
const TABS = [
  ...
  },
  {
    ...
    name: "Payments",
    icon: RiBankCardLine,
    desc: "Stripe & Tax Settings",
  },
  {
    id: "connectivity",
    name: "Email Server",
    icon: RiServerLine,
    desc: "SMTP Server Config",
  },
...
const SuperadminSettings = () => {
  ...
  const { modules, modulesLoading, fetchAllModules, addModule, removeModule } =
    useModules();
  ...
  const handleSaveStripe = async () => {
    ...
    try {
      await configureGateway({
        ...
        free_trial_days: stripeConfig.free_trial_enabled
          ? Number(stripeConfig.free_trial_days)
          : undefined,
      });
  ...
  const handleAddModule = async () => {
    ...
    else if (!/^[a-z]+\.[a-z0-9-]+$/.test(newModule.key.trim()))
      errors.key = "Format: panel.name (e.g. admin.reports)";
    ...
    if (result.ok) {
      ...
      toast.error(
        result.error?.response?.data?.message || "Failed to create module",
      );
    }
  };
  ...
  const ActiveIcon =
    TABS.find((t) => t.id === activeTab)?.icon || RiSettings4Line;
    ...
    <div className="space-y-4 pb-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-gray-100">
        ...
        {activeTab !== "modules" && !SELF_SAVING_TABS.has(activeTab) && (
          <Button
            variant="primary"
            className="text-[10px] font-black uppercase tracking-widest px-6 py-2.5 shadow-lg shadow-primary/20"
          >
        ...
        {activeTab !== "modules" && !SELF_SAVING_TABS.has(activeTab) && (
          <div className="sticky bottom-0 px-4 py-3 bg-gray-50/90 backdrop-blur-sm border-t border-gray-100 flex items-center justify-between rounded-b-xl z-20">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black text-gray-400 uppercase tracking-widest tracking-tighter">
                System Health: Optimal
              </span>
            </div>
            <Button
              variant="primary"
              className="text-xs font-black uppercase tracking-widest px-8 shadow-md"
            >
              Push Configuration
            </Button>
          </div>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2 w-full">
            ...
            <Button
              className="bg-red-500 hover:bg-red-600 border-red-600 px-6 py-2 text-sm font-bold"
      ...
      >
        <div className="p-4 bg-red-50 border border-red-100 rounded-lg flex gap-3">
          ...
          <p className="text-sm text-red-800 font-bold leading-tight">
            ...
            ? It will no longer appear in plan module assignments.
          </p>
        </div>
      </Modal>
    </div>
