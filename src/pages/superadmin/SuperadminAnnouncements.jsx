...
import {
  ...
  exportSuperadminAnnouncements,
} from "../../services/superadminAnnouncement.service";

const FORM_ID = "superadmin-announcement-form";
const PAGE_SIZE = 10;
...
export default function SuperadminAnnouncements() {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await exportSuperadminAnnouncements();
      const url = URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement("a");
      a.href = url;
      a.download = `Platform_Announcements_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const loadList = useCallback(async (nextPage = 1) => {
    setLoadingList(true);
    try {
      const res = await listSuperadminAnnouncements({ page: nextPage, limit: PAGE_SIZE });
  ...
  const openEdit = (a) => {
    ...
    setTarget(a.target === "selected" ? "selected" : "all");
    setSelectedOrgIds(Array.isArray(a.org_ids) ? a.org_ids : []);
  ...
  const handleDelete = async (a) => {
    ...
    setDeletingId(a.id);
    try {
      await deleteSuperadminAnnouncement(a.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = editingId
        ? await updateSuperadminAnnouncement(editingId, payload)
        : await sendSuperadminAnnouncement(payload);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 py-6">
        ...
        {loadingList ? (
          ...
          <div className="py-14 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Click “New announcement” to send your first platform-wide message.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-gray-500 bg-slate-50/60 border-b border-slate-100">
                  <th className="py-2.5 px-5 font-bold">Date</th>
                  <th className="py-2.5 px-5 font-bold">Title</th>
                  <th className="py-2.5 px-5 font-bold">Audience</th>
                  <th className="py-2.5 px-5 font-bold text-right">Orgs</th>
                  <th className="py-2.5 px-5 font-bold text-right">Recipients</th>
                  <th className="py-2.5 px-5 font-bold text-center">Email</th>
                  <th className="py-2.5 px-5 font-bold">Sent by</th>
              ...
              <tbody>
                {items.map((a) => (
                  <tr
                    key={a.id}
                    className="border-b border-slate-50 last:border-0 align-top hover:bg-slate-50/50"
                  >
                    <td className="py-3 px-5 whitespace-nowrap text-gray-500">
                    ...
                    <td className="py-3 px-5 text-right text-secondary">
                      {a.organisations_count ?? 0}
                    </td>
                    <td className="py-3 px-5 text-right font-semibold text-secondary">
                      {a.recipients ?? 0}
                    </td>
                    ...
                    <td className="py-3 px-5 text-gray-500">{a.created_by_email || "—"}</td>
                    <td className="py-3 px-5">
      ...
      <Modal
        ...
        subtitle={
          editingId
            ? "Saving re-sends the notification (and email) to organisation admins"
        ...
        <form id={FORM_ID} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={5}
              className="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-y min-h-[120px]"
          ...
          <div>
            <p className="text-xs font-bold text-gray-600 mb-2 uppercase tracking-wide">Send to</p>
          ...
          {target === "selected" && (
            <div className="rounded-2xl border border-dashed border-gray-200 p-3 max-h-44 overflow-y-auto space-y-1.5 custom-scrollbar">
              {loadingOrgs ? (
                ...
                orgs.map((org) => (
                  <label
                    key={org.id}
                    className="flex items-center gap-3 py-1.5 px-1 cursor-pointer text-sm hover:bg-gray-50 rounded-lg"
                  >
                    <input
                      type="checkbox"
