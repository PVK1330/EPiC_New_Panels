...
const StatCard = ({ title, value, subtitle, icon: Icon, color, delay = 0 }) => (
  ...
  >
    <div className="flex items-center justify-between">
      ...
      <div
        className={`p-2 rounded-xl border transition-all duration-300 group-hover:scale-110 ${color}`}
      >
        <Icon size={18} />
      </div>
    </div>
...
const SuperadminDashboard = () => {
  ...
  const loadStats = useCallback(async (isRefresh = false) => {
    try {
      ...
      const [res, notifRes] = await Promise.all([
        fetchDashboardStats(),
        fetchPlatformNotifications({ limit: 4 }).catch(() => ({
          data: { notifications: [] },
        })),
      ]);
  ...
  const planTotal =
    planDist.reduce((s, p) => s + p.orgCount, 0) + (stats?.orgsWithNoPlan || 0);
    ...
    <div className="space-y-6 pb-6 animate-in fade-in slide-in-from-bottom-3 duration-300 print-dashboard">
      ...
      {!loading && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <MiniStat
            label="Suspended"
            value={orgs.suspended || 0}
            icon={RiCloseCircleLine}
            iconColor="text-rose-500"
          />
          <MiniStat
            label="Gross Volume"
            value={formatCurrency(revenue.grossVolume, currency)}
            icon={RiExchangeLine}
            iconColor="text-primary"
          />
          <MiniStat
            label="Pending Invoices"
            value={invoices.pending || 0}
            icon={RiAlertLine}
            iconColor="text-amber-500"
          />
          <MiniStat
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-[10px] text-slate-600 font-bold uppercase tracking-wide mt-1">
                  Monthly platform revenue volume
                </p>
              </div>
              <div className="text-right">
                <p className="text-xl font-black text-secondary">
                  {formatCurrency(revenue.netRevenue, currency)}
                </p>
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">
                  Net volume this month
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            <div className="flex-1 min-h-[240px] flex items-center justify-center text-slate-400 text-xs font-bold uppercase tracking-widest">
              Loading chart...
            </div>
          ) : monthlyRev.length === 0 ? (
            <div className="flex-1 min-h-[240px] flex items-center justify-center text-slate-400 text-xs font-bold uppercase tracking-widest">
            ...
            <div className="mt-6">
              ...
              <div className="flex justify-between mt-3">
                {monthlyRev.map((m) => (
                  <span
                    key={m.month}
                    className="text-[9px] font-black text-slate-600 flex-1 text-center truncate uppercase tracking-widest"
                  >
                    {m.month}
                  </span>
                ))}
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <div className="space-y-1.5 w-full">
                {planDist.map((plan, i) => (
                  <div
                    key={plan.id}
                    className="flex items-center justify-between p-2 rounded-xl border border-gray-100 bg-gray-50/20 text-xs font-bold text-slate-700"
                  >
                    <div className="flex items-center gap-2.5">
                      <div
                        className={`w-2.5 h-2.5 rounded-full ${PLAN_COLORS[i % PLAN_COLORS.length]}`}
                      />
                      <span>{plan.name}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-secondary">
                      ...
                      <span className="text-[10px] text-slate-500 font-medium">
                        (
                        {planTotal > 0
                          ? ((plan.orgCount / planTotal) * 100).toFixed(0)
                          : 0}
                        %)
                      </span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 lg:self-start bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/20">
            <div>
              ...
              <p className="text-[10px] text-slate-600 font-bold uppercase tracking-wide mt-0.5">
                Newly onboarded system tenants
              </p>
            </div>
            <button
              ...
              <table className="w-full min-w-0 text-xs">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-gray-50 border-b border-gray-150 text-left text-slate-600 font-bold">
                    <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest">
                      Sr No
                    </th>
                    <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest">
                      Organization
                    ...
                    <th className="px-5 py-3 text-center text-[9px] font-black uppercase tracking-widest">
                      Users
                    </th>
                    <th className="px-5 py-3 text-center text-[9px] font-black uppercase tracking-widest">
                      Status
                ...
                <tbody className="divide-y divide-gray-100 font-bold text-slate-700">
                  {recentOrgs.map((org, index) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[9px] text-slate-500 font-medium lowercase block mt-0.5">
                              {org.slug}
                            </span>
                          </div>
                        </div>
                      ...
                      <td className="px-5 py-3 text-center">
                        <span
                          className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border capitalize ${STATUS_BADGE[org.status] || "bg-gray-50 text-gray-550 border-gray-200"}`}
                        >
                          {org.status}
                        </span>
                      </td>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            ...
            <div className="p-5 flex-1 flex flex-col justify-between gap-4">
              <div className="space-y-3">
                ...
                />
                <SummaryRow
                  label="Net Revenue (97%)"
                  value={formatCurrency(revenue.netRevenue, currency)}
                />
                <SummaryRow label="Transactions" value={txns.total || 0} />
                <SummaryRow
                  label="Success Rate"
                  value={`${txns.successRate || 0}%`}
                />
                <SummaryRow
                  label="Refund Rate"
                  value={`${txns.refundRate || 0}%`}
                />
              </div>
              ...
              <div className="border-t border-gray-100 pt-4 mt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <SubBadge
                    label="Expired"
                    value={subs.expired || 0}
                    color="bg-slate-50 text-slate-650 border border-slate-200"
                  />
                  <SubBadge
                    label="Cancelled"
                    value={subs.cancelled || 0}
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/20 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-650 font-bold uppercase tracking-wide mt-0.5">
                  ...
                  {notifications.map((n) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-600 font-semibold line-clamp-1 mt-0.5">
                          {n.desc}
                        </p>
...
function MiniStat({ label, value, icon: Icon, iconColor }) {
  return (
    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-gray-200/80 shadow-sm flex items-center gap-3">
      <div
        className={`p-1.5 rounded-lg bg-gray-50 border border-gray-100 shrink-0 ${iconColor}`}
      >
        <Icon size={16} />
      </div>
      <div>
...
function SubBadge({ label, value, color }) {
  return (
    <div
      className={`p-3 rounded-xl text-center flex flex-col justify-center ${color}`}
    >
      <span className="text-lg font-black tracking-tight block leading-none">
        {value}
      </span>
      <span className="text-[9px] font-black uppercase tracking-widest opacity-80 mt-1 block">
        {label}
      </span>
    </div>
  );
