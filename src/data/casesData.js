export const INITIAL_CASES = [
  {
    caseId: "CAS-001",
    candidate: "John Smith",
    business: "Tech Solutions Ltd",
    visaType: "H-1B",
    status: "Approved",
    submitted: "2024-01-15",
    priority: "high",
    caseworker: "Emily Davis · Sarah Chen",
    caseworkerIds: ["CW-301", "CW-304"],
    caseworkerNames: ["Emily Davis", "Sarah Chen"],
    totalAmount: 5000,
    paidAmount: 5000,
    notes: "All documents submitted.",
    candidateId: "C-1001",
    businessId: "B-2001",
    caseworkerId: "CW-301, CW-304",
    targetSubmissionDate: "2024-02-01",
    nationality: "American",
    jobTitle: "Software Engineer",
    department: "Engineering",
    salaryOffered: 120000,
    petitionType: "New",
    lcaNumber: "I-200-24001",
    receiptNumber: "EAC2401234567",
  },
  {
    caseId: "CAS-002",
    candidate: "Sarah Johnson",
    business: "Global Tech Inc",
    visaType: "L-1A",
    status: "Pending",
    submitted: "2024-01-18",
    priority: "medium",
    caseworker: "Mark Lee",
    caseworkerIds: ["CW-302"],
    caseworkerNames: ["Mark Lee"],
    totalAmount: 4500,
    paidAmount: 2000,
    notes: "Awaiting additional docs.",
    candidateId: "C-1002",
    businessId: "B-2002",
    caseworkerId: "CW-302",
    targetSubmissionDate: "2024-02-15",
    nationality: "Canadian",
    jobTitle: "Product Manager",
    department: "Product",
    salaryOffered: 135000,
    petitionType: "Extension",
    lcaNumber: "I-200-24002",
    receiptNumber: "",
  },
];

export const getCaseById = (caseId) => {
  return INITIAL_CASES.find(c => c.caseId === caseId);
};
